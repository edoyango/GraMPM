    ...
    template<typename F> void MPM_system<F>::save_to_file(const std::string &prefix, const int &timestep) const {
        ...
        for (int i = 0; i < m_p_size; ++i) {
            outfile << std::setw(i_width) << i << ' ' << std::setprecision(f_precision)
                    ...
                    << std::setw(f_width) << std::fixed << m_p_sigmaxx[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_sigmayy[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_sigmazz[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_sigmaxy[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_sigmaxz[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_sigmayz[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_ax[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_ay[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_az[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_dxdt[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_dydt[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_dzdt[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_strainratexx[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_strainrateyy[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_strainratezz[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_strainratexy[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_strainratexz[i] << ' '
                    << std::setw(f_width) << std::fixed << m_p_strainrateyz[i] << ' '
